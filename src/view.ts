...
async function saveEvent(): Promise<any> {
    const username: any = await api.queryDatabase("SELECT username FROM user WHERE id=(?)", user_ID);
    const currentdate: any = new Date().toISOString().substr(0, 10);
    const dsInput: any = document.getElementById("descriptionInput");
    document.getElementById("saveKnop")?.addEventListener("click", async function () {
        api.queryDatabase(
            `INSERT INTO event (description, dateCreated) VALUES ('${dsInput.value}', '${currentdate}')`
        ).then(()=>{
            const eventId: any = api.queryDatabase(
                `SELECT eventID FROM event WHERE description='${dsInput.value}'`
            );
            console.log(eventId);
        });
